...
const CustomNumberInput = React.forwardRef(function CustomNumberInput(props, ref) {
  ...
  // Ensure that the inputProps are handling the value and onChange correctly
  inputProps.value = value;
  inputProps.onChange = (e) => onChange(Number(e.target.value));

  const incrementProps = getIncrementButtonProps();
  const decrementProps = getDecrementButtonProps();

  // Ensure that the increment and decrement buttons are correctly wired
  incrementProps.onClick = (e) => {
    e.preventDefault();
    onChange(value + 1);
  };

  decrementProps.onClick = (e) => {
    e.preventDefault();
    onChange(value - 1);
  };
  ...
  return (
    <StyledInputRoot {...getRootProps()}>
      <StyledStepperButton {...incrementProps} className="increment">
        ▴
      </StyledStepperButton>
